...
async def on_message(message: discord.Message):
    ...
    # ── Track Quilly's messages ──
    if message.author.id == WATCH_USER_ID:
        now = datetime.now(timezone.utc)
        today = now.date()
        if quilly_state["messages_today_date"] != today:
            quilly_state["messages_today"] = 0
            quilly_state["messages_today_date"] = today
        quilly_state["messages_today"] += 1
        quilly_state["last_message_at"] = now
        quilly_state["last_message_content"] = message.content[:100]
        quilly_state["last_message_channel"] = message.channel.name
    ...
    # ── What was my Quilly up to? ──
    if message.content == "What was my Quilly up to?" and message.author.id == NOTIFY_USER_ID:
        now = datetime.now(timezone.utc)
        guild = bot.guilds[0]
        member = guild.get_member(WATCH_USER_ID)
        ...
        current_status = status_labels.get(
            member.status if member else quilly_state["status"],
            "Unknown"
        )

        if quilly_state["status_changed_at"]:
            diff = now - quilly_state["status_changed_at"]
            mins = int(diff.total_seconds() // 60)
            hrs  = mins // 60
            status_duration = f"{hrs}h {mins % 60}m" if hrs > 0 else f"{mins}m"
        else:
            status_duration = "Unknown"
        ...
        embed.add_field(
            name="Last Online",
            value=time_ago(quilly_state["last_online_at"]),
            inline=True,
        )
        embed.add_field(
            name="Activity",
            value=quilly_state["activity"] or "Nothing",
            inline=True,
        )
        if quilly_state["spotify"]:
            sp = quilly_state["spotify"]
            embed.add_field(
                name="🎵 Spotify",
                value=f"**{sp['song']}**\nby {sp['artist']}\n*{sp['album']}*\n{time_ago(sp['at'])}",
                inline=True,
            ...
            embed.add_field(name="🎵 Spotify", value="Not listening", inline=True)

        if quilly_state["voice_channel"]:
            embed.add_field(
                name="🔊 Voice Channel",
                value=f"**{quilly_state['voice_channel']}**\n*joined {time_ago(quilly_state['voice_joined_at'])}*",
                inline=True,
            )
        else:
            embed.add_field(name="🔊 Voice Channel", value="Not in one", inline=True)

        embed.add_field(
            name="💬 Messages Today",
            value=str(quilly_state["messages_today"]),
            inline=True,
        )
        if quilly_state["last_message_at"]:
            embed.add_field(
                name="Last Message",
                value=f"*{time_ago(quilly_state['last_message_at'])}* in **#{quilly_state['last_message_channel']}**\n> {quilly_state['last_message_content']}",
                inline=False,
            )
        if quilly_state["status_history"]:
            history_lines = "\n".join(
                f"{time_ago(e['at'])} — {e['from']} → {e['to']}"
                for e in reversed(quilly_state["status_history"])
            )
            embed.add_field(
                name="📜 Recent Status Changes",
                value=history_lines,
                inline=False,
            )
        else:
            embed.add_field(
                name="📜 Recent Status Changes",
                value="None recorded yet (bot tracks from startup)",
                inline=False,
            )

        embed.set_footer(text="Tracking since bot started — history resets on restart")
        if member:
            embed.set_thumbnail(url=member.display_avatar.url)
